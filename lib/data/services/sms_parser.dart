...
class SmsParser {
  ...
  static ParsedSms parse(String message) {
    ...
    if (deliverMatch != null) {
      final qty = int.tryParse(deliverMatch.group(1) ?? '') ?? 0;
    ...
    if (dropMatch != null) {
      final qty = int.tryParse(dropMatch.group(1) ?? '') ?? 0;
      if (qty < AppConstants.minQuantity || qty > AppConstants.maxQuantity) {
        return ParsedSms(
          command: SmsCommand.unknown,
          quantity: qty,
  ...
  static String getUnknownCommandReply() {
    return 'Invalid. Use DELIVER [${AppConstants.minQuantity}-${AppConstants.maxQuantity}] or DROP [qty] where qty is ${AppConstants.minQuantity} or more.';
  }
}
